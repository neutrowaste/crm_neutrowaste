...
//   created_at: timestamp with time zone (not null, default: now())
//   updated_at: timestamp with time zone (not null, default: now())
//   vigencia: text (nullable)
//   objeto: text (nullable)
//   data_inicio: date (nullable)
//   data_termino: date (nullable)
//   nome_gestor: text (nullable)
//   telefone_gestor: text (nullable)
// Table: documentacoes
//   id: uuid (not null, default: gen_random_uuid())
...
// --- CONSTRAINTS ---
// Table: app_roles
//   UNIQUE app_roles_name_key: UNIQUE (name)
//   PRIMARY KEY app_roles_pkey: PRIMARY KEY (id)
// Table: app_settings
...
// Table: app_roles
//   Policy "Allow admin modify app_roles" (ALL, PERMISSIVE) roles={authenticated}
//     USING: is_admin()
//     WITH CHECK: is_admin()
//   Policy "Allow authenticated read app_roles" (SELECT, PERMISSIVE) roles={authenticated}
//     USING: true
// Table: app_settings
...
//     WITH CHECK: true
// Table: contract_templates
//   Policy "Allow authenticated full contract_templates" (ALL, PERMISSIVE) roles={authenticated}
//     USING: true
//     WITH CHECK: true
// Table: contracts
//   Policy "Allow authenticated users to delete their own contracts or all " (DELETE, PERMISSIVE) roles={authenticated}
//     USING: (is_admin() OR (uploaded_by = auth.uid()))
//   Policy "Allow authenticated users to insert contracts" (INSERT, PERMISSIVE) roles={authenticated}
//     WITH CHECK: true
//   Policy "Allow authenticated users to read their own contracts or all if" (SELECT, PERMISSIVE) roles={authenticated}
//     USING: (is_admin() OR (uploaded_by = auth.uid()))
//   Policy "Allow authenticated users to update their own contracts or all " (UPDATE, PERMISSIVE) roles={authenticated}
//     USING: (is_admin() OR (uploaded_by = auth.uid()))
...
//     WITH CHECK: true
// Table: leads
//   Policy "Allow authenticated users to delete their own leads or all if a" (DELETE, PERMISSIVE) roles={authenticated}
//     USING: (is_admin() OR (assigned_to = auth.uid()))
//   Policy "Allow authenticated users to insert leads" (INSERT, PERMISSIVE) roles={authenticated}
//     WITH CHECK: true
//   Policy "Allow authenticated users to read their own leads or all if adm" (SELECT, PERMISSIVE) roles={authenticated}
//     USING: (is_admin() OR (assigned_to = auth.uid()))
//   Policy "Allow authenticated users to update their own leads or all if a" (UPDATE, PERMISSIVE) roles={authenticated}
//     USING: (is_admin() OR (assigned_to = auth.uid()))
// Table: logs
...
// --- DATABASE FUNCTIONS ---
// FUNCTION get_public_contract(uuid)
