...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (supabaseUrl && supabaseKey) {
      ...
      if (smtpSettings) {
        console.log(
          `[Email Service] Utilizando servidor SMTP customizado: ${smtpSettings.host}:${smtpSettings.port}`,
        )
        console.log(
          `[Email Service] Remetente: ${smtpSettings.from_name} <${smtpSettings.from_email}>`,
        )
      } else {
        console.log(`[Email Service] Utilizando serviço de envio padrão.`)
      }
    ...
    if (!subject || !body) {
      if (type === 'welcome') {
        ...
        body = `Olá ${data?.name || ''},\n\nO contrato para a empresa ${data?.company || ''} foi gerado e está aguardando sua assinatura. Por favor, acesse o portal para revisá-lo e assiná-lo.\n\nAtenciosamente,\nEquipe Neutrowaste`
      } else {
