  ...
  try {
    ...
    if (smtpSettings && smtpSettings.is_active && smtpSettings.host) {
      ...
      return new Response(
        JSON.stringify({
          success: true,
          message: `Email do tipo '${type}' enviado com sucesso via SMTP.`,
        }),
        {
      ...
      console.log(
        `[Email Service] ERRO: Servidor SMTP não configurado ou inativo.`,
      )
      throw new Error(
        'Servidor de e-mail SMTP não configurado ou inativo. Acesse as configurações para habilitar o envio.',
      )
    }
  } catch (error: any) {
