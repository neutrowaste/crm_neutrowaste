...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (smtpSettings && smtpSettings.is_active && smtpSettings.host) {
      ...
      console.log(`[Email Service] AVISO: Servidor SMTP não configurado ou inativo. Simulando envio de e-mail.`);
      return new Response(JSON.stringify({ success: true, message: `Email do tipo '${type}' simulado com sucesso (SMTP não configurado).` }), {
        headers: { 'Content-Type': 'application/json', ...corsHeaders },
      });
    }
