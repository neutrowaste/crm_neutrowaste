...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (supabaseUrl && supabaseKey) {
      ...
      const { data: fetchedSmtpSettings, error: fetchError } =
        await supabaseAdmin.from('smtp_settings').select('*').limit(1).single()

      if (fetchError && fetchError.code !== 'PGRST116') {
        console.error(
          '[Email Service] Erro ao buscar configurações de email:',
          fetchError,
        )
      }
    ...
    if (!subject || !body) {
      if (type === 'welcome') {
        ...
        subject = `Aviso: O contrato ${data?.contractName || ''} se encerra em ${data?.days || ''} dias`
        body = `Olá ${data?.name || ''},\n\nEste é um aviso automático informando que o contrato "${data?.contractName || ''}" (Cliente: ${data?.company || ''}) irá vencer em exatamente ${data?.days || ''} dias.\n\nPor favor, tome as ações necessárias para renovação ou encerramento.\n\nAtenciosamente,\nSistema Neutrowaste`
      } else {
    ...
    const isActive =
      !!envResendApiKey || (smtpSettings && smtpSettings.is_active)

    if (!isConfigured || !isActive) {
      console.log(
        `[Email Service] AVISO: Servidor de e-mail não configurado ou inativo.`,
      )
      return new Response(
        JSON.stringify({
          error: `Servidor de e-mail não configurado ou inativo. Acesse as configurações para habilitar o envio.`,
        }),
        {
          status: 500,
    ...
    if (!resendApiKey.startsWith('re_')) {
      return new Response(
        JSON.stringify({
          error: `A API Key do Resend é inválida ou não foi configurada. Verifique as configurações.`,
        }),
        {
          status: 500,
          headers: { 'Content-Type': 'application/json', ...corsHeaders },
    ...
    if (!res.ok) {
      throw new Error(
        resData.message || 'Falha ao enviar e-mail via Resend API',
      )
    }

    console.log(
      `[Email Service] E-mail enviado com sucesso via Resend para ${email}`,
    )

    return new Response(
      JSON.stringify({
        success: true,
        message: `Email do tipo '${type}' enviado com sucesso via Resend API.`,
        id: resData.id,
      }),
      {
        headers: { 'Content-Type': 'application/json', ...corsHeaders },
